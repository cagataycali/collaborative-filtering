module.exports = function () {
  ...
  app.recommendByArray = (listenList, count) => {
    ...
    for (let i = 0; i < listenList.length; i++) {
      ...
      for (let j = 0; j < similarUserList.length; j++) {
        ...
        similarUsers[similarUserList[j].userId] +=
          similarUserList[j].feature * listenList[i].feature;
      }
    }
    ...
    for (let i = 0; i < relatedUsers.length; i++) {
      if (app.maxRelatedUser !== 0 && i > app.maxRelatedUser) break;
      ...
      for (let j = 0; j < userList.length; j++) {
        ...
        if (app.maxRelatedItem !== 0 && j > app.maxRelatedItem) break;
    ...
    for (let itemId in playlist)
      result.push({
        itemId: itemId,
        score: Math.round(Math.log(playlist[itemId] + 1) * 100) / 100,
      });
    ...
    for (let i = 0; i < trained.ranking.length; i++) {
      ...
      if (!playlist[trained.ranking[i].itemId]) result.push(trained.ranking[i]);
    }
  ...
  app.recommendToUser = (userId, count) => {
    let userList = trained.userBase[userId];
    if (userList) return app.recommendByArray(userList, count);
    else return JSON.parse(JSON.stringify(trained.ranking)).splice(0, count);
  };
